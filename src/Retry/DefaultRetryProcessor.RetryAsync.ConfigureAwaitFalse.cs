...
namespace PoliNorError
{
	public sealed partial class DefaultRetryProcessor
	{
		...
		public Task<PolicyResult<T>> RetryInfiniteWithErrorContextAsync<TErrorContext, T>(Func<CancellationToken, Task<T>> func, TErrorContext param, CancellationToken token)
		{
			return RetryWithErrorContextAsync(func, param, RetryCountInfo.Infinite(), token);
		}
		...
		{
			return RetryAsync(func, param, retryCountInfo, false, token);
		}

		public Task<PolicyResult<T>> RetryAsync<TParam, T>(Func<TParam, CancellationToken, Task<T>> func, TParam param, RetryCountInfo retryCountInfo, CancellationToken token)
		{
			return RetryAsync(func, param, retryCountInfo, false, token);
		}
		...
		public Task<PolicyResult<T>> RetryInfiniteAsync<TParam, T>(Func<TParam, CancellationToken, Task<T>> func, TParam param, CancellationToken token)
		{
			return RetryAsync(func, param, RetryCountInfo.Infinite(), token);
		}
	}
