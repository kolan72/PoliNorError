...
{
	/// <summary>
	/// Provides an abstract base class for implementing custom error processing logic that operates on inner exceptions
	/// of a specific type <typeparamref name="TException"/>.
	/// </summary>
	/// <typeparam name="TException">
	/// The specific type of <see cref="Exception"/> that this processor is designed to handle as an inner exception.
	/// </typeparam>
	/// <remarks>
	/// <para>
	...
	/// <see cref="Execute"/> method, which derived classes must implement for the specific inner exception type.
	/// </para>
	/// <para>
	/// The processor only executes when the exception's <see cref="Exception.InnerException"/> property contains
	/// an exception of the exact type <typeparamref name="TException"/>.
	/// </para>
	/// </remarks>
	public abstract class InnerErrorProcessor<TException> : IErrorProcessor where TException : Exception
	{
		private readonly DefaultInnerErrorProcessor<TException> _errorProcessor;

		/// <summary>
		/// Initializes a new instance of the <see cref="InnerErrorProcessor{TException}"/> class.
		/// </summary>
		/// <remarks>
		/// The constructor wires the <see cref="Execute"/> method into the internal
		/// <see cref="DefaultInnerErrorProcessor{TException}"/> pipeline.
		/// </remarks>
		protected InnerErrorProcessor()
		{
			_errorProcessor = new DefaultInnerErrorProcessor<TException>(Execute);
		}

		/// <summary>
		/// Processes the given exception synchronously by invoking the internal inner error processor.
		/// </summary>
		...
		/// eventually calling the overridden <see cref="Execute"/> method if the inner exception type matches.
		/// </remarks>
		...
		/// <summary>
		/// The core processing logic for the specific inner exception type. This method must be implemented by inheriting classes.
		/// </summary>
		/// <param name="error">The typed inner exception to be processed.</param>
		/// <param name="catchBlockProcessErrorInfo">Optional information about the context in which the exception was caught.</param>
		/// <param name="token">A cancellation token.</param>
		/// <remarks>
		/// <strong>Inheritors of this class MUST implement this method</strong> to define the specific logic
		/// for handling inner exceptions of type <typeparamref name="TException"/>, such as specialized logging or alerting.
		/// This method is only called when the outer exception's <see cref="Exception.InnerException"/> is of the exact type <typeparamref name="TException"/>.
		/// </remarks>
