...
namespace PoliNorError.Tests
{
	internal class DelegateExtensionsTRetryTests
	{
		...
		public void Should_InvokeWithRetryAndDelayInfiniteWithFunc_Work()
		{
			...
			int k = 0;
			Func<int> action2 = () => { k++; throw new Exception(); };
		...
		[Test]
		public async Task Should_InvokeWithRetryAndDelayInfiniteAsyncWithFunc_Work()
		{
			int i = 0;
			Func<CancellationToken, Task<int>> fn = async (_) => { await Task.Delay(1); i++; throw new Exception(); };
