...
namespace PoliNorError.Tests
{
	internal class PolicyDelegateTCollectionTests
	{
		...
		public void Should_WithRetry_With_RetryDelay_Param_Adds_Policy_Correctly()
		{
			...
			int actSave() { throw new Exception("Test"); }
			var policyDelegateCollection = PolicyDelegateCollection<int>.Create(new RetryPolicy(2), actSave).WithRetry(1, delay).AndDelegate(actSave);
		...
		public void Should_WithInfiniteRetry_With_RetryDelay_Param_Adds_Policy_Correctly()
		{
			var delay = new ConstantRetryDelay(TimeSpan.FromSeconds(1));
			int actSave() { throw new Exception("Test"); }
			var policyDelegateCollection = PolicyDelegateCollection<int>.Create(new RetryPolicy(2), actSave).WithInfiniteRetry(delay).AndDelegate(actSave);
		...
		public async Task Should_IncludeErrorSet_ForInnerExceptions_With_IErrorSetParam_Work(TestErrorSetMatch testErrorSetMatch, bool isFailed, bool consistsOfErrorAndInnerError)
		{
			...
			var policyDelegateCollection = PolicyDelegateCollection<int>
						.Create()
						.WithRetry(1).AndDelegate(TestHandlingForErrorSet.GetTwoGenericParamFunc(testErrorSetMatch, null, true))
						.WithRetry(1).AndDelegate(TestHandlingForErrorSet.GetTwoGenericParamFunc(testErrorSetMatch, null, true))
						.IncludeErrorSet(errorSet);
