...
namespace PoliNorError.Tests
{
	internal class PolicyDelegateTCollectionTests
	{
		...
		public async Task Should_Handle_For_FallbackFuncInExtensions_Work()
		{
			...
			Assert.AreEqual(PolicyDelegateCollectionResultStatus.LastPolicySuccess, res.GetStatus());
		}
		...
		public void Should_AddPolicyDelegate_After_WithCommonDelegate_Work_ForAsyncFunc()
		{
			...
			int i = 0;
			async Task<int> taskSave(CancellationToken _) { i++; await Task.Delay(1); throw new Exception("Test"); }

			policyDelegateCollection.WithCommonDelegate(taskSave).WithRetry(1).AndDelegate(taskSave);
