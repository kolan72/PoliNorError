...
namespace PoliNorError.Tests
{
	internal class DelegateExtensionsRetryTests
	{
		...
		public void Should_InvokeWithRetry_Work()
		{
			...
			action.InvokeWithRetry(retryCount, RetryErrorProcessor.From(actionError));
			...
			action.InvokeWithRetry(retryCount, RetryErrorProcessor.From(beforeProcessErrorAsync, ConvertToCancelableFuncType.Cancelable));
		...
		public void Should_InvokeWithRetryWithDelay_Work()
		{
			...
			action.InvokeWithWaitAndRetry(retryCount, TimeSpan.FromSeconds(0), RetryErrorProcessor.From(actionError));
			Assert.AreEqual(1, i1);
			...
			int i3 = 0;
			Task beforeProcessErrorAsync(Exception _) { i3++; return Task.CompletedTask; }
			action.InvokeWithWaitAndRetry(retryCount, TimeSpan.FromSeconds(0), RetryErrorProcessor.From(beforeProcessErrorAsync, ConvertToCancelableFuncType.Cancelable));
			Assert.AreEqual(1, i3);
		...
		public void Should_InvokeWithRetryWithFunc_Work()
		{
			...
			action.InvokeWithWaitAndRetry(retryCount, retryFunc, RetryErrorProcessor.From(actionError));
			Assert.AreEqual(1, i1);
			...
			int i3 = 0;
			Task beforeProcessErrorAsync(Exception _) { i3++; return Task.CompletedTask; }
			action.InvokeWithWaitAndRetry(retryCount, retryFunc, RetryErrorProcessor.From(beforeProcessErrorAsync, ConvertToCancelableFuncType.Cancelable));
			Assert.AreEqual(1, i3);
		...
		public async Task Should_InvokeWithRetryAsync_Work()
		{
			...
			int i1 = 0;
			void actionError(Exception _) { i1++; }
			await func.InvokeWithRetryAsync(retryCount, RetryErrorProcessor.From(actionError));
			Assert.AreEqual(1, i1);
			...
			int i3 = 0;
			Task beforeProcessErrorAsync(Exception _) { i3++; return Task.CompletedTask; }
			await func.InvokeWithRetryAsync(retryCount, RetryErrorProcessor.From(beforeProcessErrorAsync, ConvertToCancelableFuncType.Cancelable));
			Assert.AreEqual(1, i3);
		...
		public async Task Should_InvokeWithRetryWithDelayAsync_Work()
		{
			...
			int i1 = 0;
			void actionError(Exception _) { i1++; }
			await func.InvokeWithWaitAndRetryAsync(retryCount, TimeSpan.FromSeconds(0), RetryErrorProcessor.From(actionError));
			Assert.AreEqual(1, i1);
			...
			int i3 = 0;
			Task beforeProcessErrorAsync(Exception _) { i3++; return Task.CompletedTask; }
			await func.InvokeWithWaitAndRetryAsync(retryCount, TimeSpan.FromSeconds(0), RetryErrorProcessor.From(beforeProcessErrorAsync, ConvertToCancelableFuncType.Cancelable));
			Assert.AreEqual(1, i3);
		...
		public async Task Should_InvokeWithRetryWithFuncAsync_Work()
		{
			...
			int i1 = 0;
			void actionError(Exception _) { i1++; }
			await func.InvokeWithWaitAndRetryAsync(retryCount, retryFunc, RetryErrorProcessor.From(actionError));
			Assert.AreEqual(1, i1);
			...
			int i3 = 0;
			Task beforeProcessErrorAsync(Exception _) { i3++; return Task.CompletedTask; }
			await func.InvokeWithWaitAndRetryAsync(retryCount, retryFunc, RetryErrorProcessor.From(beforeProcessErrorAsync, ConvertToCancelableFuncType.Cancelable));
			Assert.AreEqual(1, i3);
		...
		public void Should_InvokeWithRetryInfinite_Work()
		{
			...
			void actionError(Exception _) { i1++; }
			action.InvokeWithRetryInfinite(RetryErrorProcessor.From(actionError), cancelTokenSource2.Token);
			Assert.IsTrue(i1 > 0);
		...
		public void Should_InvokeWithWaitRetryInfinite_Work()
		{
			...
			action.InvokeWithWaitAndRetryInfinite(TimeSpan.FromSeconds(0), RetryErrorProcessor.From(actionError), cancelTokenSource2.Token);
			Assert.IsTrue(i1 > 0);
			cancelTokenSource2.Dispose();
		}
		...
		public void Should_InvokeWithWaitRetryInfinite_WithDelayFunc_Work()
		{
			...
			action.InvokeWithWaitAndRetryInfinite(retryFunc, RetryErrorProcessor.From(actionError), cancelTokenSource2.Token);
