...
namespace PoliNorError.Tests
{
	internal class DelegateExtensionsRetryTests
	{
		...
		public void Should_InvokeWithRetry_Work()
		{
			...
			action.InvokeWithRetry(retryCount, InvokeParams.From(actionError));
			...
			action.InvokeWithRetry(retryCount, InvokeParams.From(beforeProcessErrorAsync, ConvertToCancelableFuncType.Cancelable));
		...
		public void Should_InvokeWithRetryWithDelay_Work()
		{
			...
			action.InvokeWithWaitAndRetry(retryCount, TimeSpan.FromSeconds(0), InvokeParams.From(actionError));
			Assert.AreEqual(1, i1);
			...
			int i3 = 0;
			Task beforeProcessErrorAsync(Exception _) { i3++; return Task.CompletedTask; }
			action.InvokeWithWaitAndRetry(retryCount, TimeSpan.FromSeconds(0), InvokeParams.From(beforeProcessErrorAsync, ConvertToCancelableFuncType.Cancelable));
			Assert.AreEqual(1, i3);
		...
		public void Should_InvokeWithRetryWithFunc_Work()
		{
			...
			action.InvokeWithWaitAndRetry(retryCount, retryFunc, InvokeParams.From(actionError));
			Assert.AreEqual(1, i1);
			...
			int i3 = 0;
			Task beforeProcessErrorAsync(Exception _) { i3++; return Task.CompletedTask; }
			action.InvokeWithWaitAndRetry(retryCount, retryFunc, InvokeParams.From(beforeProcessErrorAsync, ConvertToCancelableFuncType.Cancelable));
			Assert.AreEqual(1, i3);
